...
class ShopScreen(ThemedScreen):
    def __init__(self, **kwargs):
        ...
        self.layout.add_widget(Label(text="MAĞAZA", font_size=sp(26), bold=True, size_hint=(1, 0.12)))
        self.coin_label = Label(text="Coin: 0", font_size=sp(18), size_hint=(1, 0.08))
        ...
        back_btn = Button(text="Geri", font_size=sp(15), size_hint=(1, 0.1),
                           background_color=(0.3, 0.3, 0.3, 1))
        back_btn.bind(on_release=lambda *a: setattr(self.manager, "current", "menu"))
        self.layout.add_widget(back_btn)
    ...
    def refresh(self):
        ...
        for theme_name, theme in THEMES.items():
            ...
            if selected:
                ...
                action_btn = Button(text="SATIN AL", font_size=sp(13), size_hint=(0.35, 1),
...
class AchievementsScreen(ThemedScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.layout = BoxLayout(orientation="vertical", padding=28, spacing=10)
        self.layout.add_widget(Label(text="BAŞARIMLAR", font_size=sp(26), bold=True, size_hint=(1, 0.12)))
...
class StatsScreen(ThemedScreen):
    def __init__(self, **kwargs):
        ...
        self.layout.add_widget(Label(text="İSTATİSTİK & REKOR", font_size=sp(22), bold=True, size_hint=(1, 0.12)))
    ...
    def on_pre_enter(self, *a):
        ...
        add_line(f"  En yuksek kombo: {stats['best_combo_ever']}")
...
class GameScreen(ThemedScreen):
    def __init__(self, **kwargs):
        ...
        self.lives_label = Label(text="Can: 3/3", font_size=sp(16), size_hint=(1, 0.05))
        root.add_widget(self.lives_label)

        self.level_label = Label(text="Seviye 1", font_size=sp(14), size_hint=(1, 0.05))
        root.add_widget(self.level_label)

        self.combo_label = Label(text="", font_size=sp(18), size_hint=(1, 0.07), color=(1, 0.6, 0.1, 1))
        root.add_widget(self.combo_label)

        root.add_widget(Label(size_hint=(1, 0.06)))

        self.question_label = Label(text="", font_size=sp(40), bold=True, size_hint=(1, 0.2))
        root.add_widget(self.question_label)

        self.answer_input = TextInput(
            text="", font_size=sp(28), multiline=False, input_filter="int",
        ...
        submit_btn = Button(text="ONAYLA", font_size=sp(19), size_hint=(1, 0.11),
                             background_color=(0.2, 0.7, 0.3, 1))
        ...
        self.feedback_label = Label(text="", font_size=sp(20), size_hint=(1, 0.1))
        root.add_widget(self.feedback_label)

        self.hint_label = Label(text="", font_size=sp(13), size_hint=(1, 0.06), color=(0.3, 0.85, 0.95, 1))
        ...
        self.continue_title = Label(text="Canin Bitti!", font_size=sp(20), bold=True, size_hint=(1, 0.3))
        self.continue_overlay.add_widget(self.continue_title)

        self.watch_ad_btn = Button(text="Reklam Izle (+1 Can)", font_size=sp(15), size_hint=(1, 0.35),
        ...
        end_now_btn = Button(text="Oyunu Bitir", font_size=sp(14), size_hint=(1, 0.3),
    ...
    def update_lives_label(self):
        self.lives_label.text = f"Can: {self.lives}/{STARTING_LIVES}"
    ...
    def refresh_joker_buttons(self):
        ...
        for key, btn in self.joker_buttons.items():
            ...
            btn.text = f"{JOKER_LABELS[key]} x{count}"
    ...
    def use_joker(self, kind):
        ...
        if kind == "dondur":
            ...
            self.hint_label.text = f"Ipucu: cevap {lower} ile {upper} arasinda"
    ...
    def show_continue_overlay(self):
        ...
        self.watch_ad_btn.text = "Reklam Izle (+1 Can)"
        self.continue_title.text = "Canin Bitti!"
    ...
    def watch_ad(self, *a):
        ...
        self.continue_title.text = f"Reklam oynatiliyor... {self._ad_countdown}"
    ...
    def submit_answer(self, *args):
        ...
        if given == self.current_answer:
            ...
            self.feedback_label.text = f"DOGRU!  +{points} PUAN"
            self.feedback_label.color = (0.3, 0.9, 0.3, 1)
            ...
            self.streak = 0
            self.combo_label.text = ""
            self.feedback_label.text = f"YANLIS! (Dogrusu: {self.current_answer})"
...
class GameOverScreen(ThemedScreen):
    def __init__(self, **kwargs):
        ...
        self.title_label = Label(text="OYUN BİTTİ", font_size=sp(26), bold=True, size_hint=(1, 0.11))
        self.score_label = Label(text="", font_size=sp(24), bold=True, size_hint=(1, 0.11))
        self.stats_label = Label(text="", font_size=sp(14), size_hint=(1, 0.2))
        self.record_label = Label(text="", font_size=sp(16), size_hint=(1, 0.09))
        self.achievements_label = Label(text="", font_size=sp(13), size_hint=(1, 0.14),
                                         color=(1, 0.85, 0.3, 1))
        ...
        retry_btn = Button(text="TEKRAR OYNA", font_size=sp(19), size_hint=(1, 0.12),
                            background_color=(0.2, 0.7, 0.3, 1))
        retry_btn.bind(on_release=self.retry)
        menu_btn = Button(text="ANA MENÜ", font_size=sp(17), size_hint=(1, 0.11),
    ...
    def show_results(self, difficulty, score, total, correct, wrong, max_combo,
        ...
        self.stats_label.text = (
            ...
            f"En yuksek kombo: {max_combo}"
        )
        if is_new_record:
            self.record_label.text = f"YENI REKOR! ({high_score})"
        ...
        if newly_unlocked:
            ...
            self.achievements_label.text = "Yeni Basarim: " + ", ".join(titles) if titles else ""
        else:
...
class HizliMatematikApp(App):
    def build(self):
        try:
            ...
            scroll = ScrollView()
            label = Label(
                text="BASLATMA HATASI - lutfen bu ekranin fotografini gonder:\n\n" + error_text,
                font_size=sp(13), size_hint_y=None, halign="left", valign="top",
                color=(1, 0.3, 0.3, 1),
            )
            label.bind(texture_size=lambda inst, size: setattr(label, "height", size[1]))
            label.bind(width=lambda inst, w: label.setter("text_size")(label, (w, None)))
